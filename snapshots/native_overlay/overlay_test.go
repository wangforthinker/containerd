...
func TestOverlay(t *testing.T) {
	...
	testsuite.SnapshotterSuite(t, "Overlay1", newSnapshotter)
}

func TestOverlayMounts(t *testing.T) {
	ctx := context.TODO()
	root, err := ioutil.TempDir("", "overlay1")
	if err != nil {
		t.Fatal(err)
	}
...
func getParents(ctx context.Context, sn snapshots.Snapshotter, root, key string) []string {
	...
	parents[0] = filepath.Join(root, "snapshots", s.ParentIDs[0], "root")
	return parents
}
...
func TestOverlayView(t *testing.T) {
	...
	expected = getParents(ctx, o, root, "/tmp/view2")[0]
	...
	if m.Source != expected {
		t.Errorf("mount source should be %s but received %q", expected, m.Source)
	}
	if len(m.Options) != 2 {
